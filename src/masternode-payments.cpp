...
void CMasternodePayments::FillBlockPayee(CBlock *pBlock, CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    if (!masternodePayments.GetBlockPayee(pindexPrev->nHeight + 1, payee, pBlock->mnvin)) {
        ...
        if (winningNode) {
            ...
            LogPrintf("CreateNewBlock: Failed to detect masternode to pay... will pay to nowhere\n");
            hasPayment = true;
        }
    }
...
bool CMasternodeBlockPayees::IsTransactionValid(const COutPoint &mnvin, const CTransaction& txNew)
{
    ...
    CBlockIndex* pindexcheck;
    CAmount nReward;
    if (pindexcheck == NULL) {
    pindexcheck = chainActive.Tip();
    }
    if(pindexcheck->IsProofOfStake()) {
     nReward = GetProofOfStakeReward(nBlockHeight);
    } else {
    nReward = GetProofOfWorkReward(nBlockHeight);
    }
    int tier = 1;
