...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (vin == CTxIn()) { //only should ask for this once
            ...
            LogPrintf("dseg - peer version %s \n",pfrom->nVersion);
            //ensure only new nodes are asked for the masternode list
            if (!isLocal && Params().NetworkID() == CBaseChainParams::MAIN /**&& pfrom->nVersion>=PROTOCOL_VERSION*/) {
