...
int64_t GetProofOfStakeReward(int nHeight)
{
    ...
    else if (nHeight > 210000 && nHeight <= 769000) {
        nSubsidy = 250 * COIN;

    }
    else if (nHeight > 769000 && nHeight <= 779000) {
        nSubsidy = 110 * COIN;
    }//premine blocks start from 779001 to 819000
    else if (nHeight > 779000 && nHeight <= 819000) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            if (coins->IsCoinBase() || coins->IsCoinStake()) {
                ...
                if (nSpendHeight - coins->nHeight < Params().COINBASE_MATURITY())
                    ...
                    if (nSpendHeight - coins->nHeight < Params().COINBASE_MATURITY_OLD())
                        return state.Invalid(
                                error("CheckInputs() : tried to spend coinbase at depth %d, coinstake=%d", nSpendHeight - coins->nHeight, coins->IsCoinStake()),
                                REJECT_INVALID, "bad-txns-premature-spend-of-coinbase");
                }
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        BOOST_REVERSE_FOREACH (CBlockIndex* pindexConnect, vpindexToConnect) {
            ...
            if (!ConnectTip(state, pindexConnect, pindexConnect == pindexMostWork ? pblock : NULL)) {
                if (state.IsInvalid()) {
                    LogPrintStr("ActivateBestChainStep X\n");
                    ...
                    LogPrintStr("ActivateBestChainStep Y\n");
                    // A system error occurred (disk space, database error, ...).
                    return false;
                }
            } else {
                LogPrintStr("ActivateBestChainStep $\n");
    ...
    do {
        ...
        while (true) {
            ...
            if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetHash() == pindexMostWork->GetBlockHash() ? pblock : NULL))
                //if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetPOWHash() == pindexMostWork->GetBlockHash() ? pblock : NULL))
                return false;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    /**if (fCheckMerkleRoot) {
         ...
         printf(" Block is %s ",block.ToString().c_str());
          printf("%s: HashMerkle %s  %s",__func__,block.hashMerkleRoot.ToString().c_str(),hashMerkleRoot2.ToString().c_str());
                ...
                BOOST_FOREACH (const CTxIn& in, tx.vin) {
                    if (mapLockedInputs.count(in.prevout)) {
                        if (mapLockedInputs[in.prevout] != tx.GetHash()) {
                            ...
                            //mapRejectedBlocks.insert(make_pair(block.GetPOWHash(), GetTime()));
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (!AcceptBlockHeader(block, state, &pindex)){
        LogPrintStr("Invalid AcceptBlock Header");
        return false;
    }/***/

    //LogPrintStr("Check Duplicate Index");
    if (pindex->nStatus & BLOCK_HAVE_DATA) {
    ...
    //if (fCheckForPruning)
    //    FlushStateToDisk(state, FLUSH_STATE_ALWAYS); // we just allocated more disk space for block files
    return true;
}
...
bool ProcessNewBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    // Preliminary checks
    bool checked = CheckBlock(*pblock, state);
    // CampusCoin: check proof-of-stake block signature
    if (!pblock->CheckBlockSignature())
        return error("ProcessNewBlock() : bad proof-of-stake block signature");
    while (true) {
        TRY_LOCK(cs_main, lockMain);
    ...
    if (!ActivateBestChain(state, pblock))
        return error("ProcessNewBlock() : ActivateBestChain failed");
    if (!fLiteMode) {
        if (masternodeSync.RequestedMasternodeAssets > MASTERNODE_SYNC_LIST) {
            masternodePayments.ProcessBlock(GetHeight() + 10);
            budget.NewBlock();
...
{
    AssertLockHeld(cs_main);
    LogPrint("debug","TestBlockValidity Block");
...
bool static LoadBlockIndexDB()
{
    ...
    LogPrintf("Load pointer to end of best chain Blocks %d \n",mapBlockIndex.size());
    ...
    LogPrintf("PruneBlockIndexCandidates\n");
    PruneBlockIndexCandidates();
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                LogPrintf("%s: Recursive reading block 0\n", __func__);
                // Recursively process earlier encountered successors of this block
                deque<uint256> queue;
                queue.push_back(hash);
                LogPrintf("%s: Start reading block 0\n", __func__);
                while (!queue.empty()) {
                    LogPrintf("%s: Start reading block 1\n", __func__);
                    uint256 head = queue.front();
                    queue.pop_front();
                    std::pair<std::multimap<uint256, CDiskBlockPos>::iterator, std::multimap<uint256, CDiskBlockPos>::iterator> range = mapBlocksUnknownParent.equal_range(head);
                    while (range.first != range.second) {
                        std::multimap<uint256, CDiskBlockPos>::iterator it = range.first;
                        LogPrintf("%s: Start reading block 2\n", __func__);
                        if (ReadBlockFromDisk(block, it->second)) {
                            ...
                            if (ProcessNewBlock(dummy, NULL, &block, &it->second)) {
                                nLoaded++;
                                //queue.push_back(block.GetPOWHash());
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (!mapBlockIndex.count(block.hashPrevBlock)) {
            if (find(pfrom->vBlockRequested.begin(), pfrom->vBlockRequested.end(), hashBlock) != pfrom->vBlockRequested.end()) {
                ...
                LogPrintf("Request block  %s %s",inv.hash.ToString(),block.hashPrevBlock.ToString());
...
int ActiveProtocol(int nHeight)
{
    if(Params().SwitchBlock()<=nHeight || Params().SwitchTime() <= GetTime()){
